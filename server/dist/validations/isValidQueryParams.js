...
const isValidQueryParams = (params) => {
    ...
    if (lang) {
        ...
        if (parsed.every((language) => {
            return (validator.isLength(language, { min: 2, max: 2 }) &&
    ...
    if (categories) {
        ...
        if (parsed.every((category) => {
            return (validator.isLength(category, { min: 1, max: 64 }) &&
    ...
    return Object.assign({}, restParams);
};
