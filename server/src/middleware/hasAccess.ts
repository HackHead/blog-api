...
const hasAccess = (right: accessRights) => {
    return async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            ...
            if (right === 'fullAccess' && storedToken.dataValues.accessRights !== 'fullAccess') {
                return res.status(403).json({
                    error: {
                        "statusCode": 403,
            ...
            Logger.error(`An unexpected error occured while validating the token, please try again: ${error}`)
